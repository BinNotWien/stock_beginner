        ...
        <div class="article">
            <div class="article-md-8">
                ...
                <table class="table-notice table-hover">
                    ...
                    if ($type == 'title') {
                        $sql = "(select * from archive where title like '%{$srch}%' AND is_deleted = 0) 
                                    union 
                                (select * from notice where title like '%{$srch}%' AND is_deleted = 0)";
                    } else {
                        $sql = "(select * from archive where {$type} = '{$srch}' AND is_deleted = 0) 
                                    union 
                                (select * from notice where {$type} = '{$srch}' AND is_deleted = 0)";
                    }
